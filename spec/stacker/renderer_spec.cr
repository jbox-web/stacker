...
describe Stacker::Renderer do
  it "support array_push function" do
    ...
    renderer.compile("spec/fixtures/array_push.j2", Hash(String, String).new).should eq("['foo', 'bar']")
  end

  it "support merge_dict function" do
    renderer = create_renderer
    renderer.compile("spec/fixtures/merge_dict.j2", Hash(String, String).new).should eq("{'foo' => {'enabled' => true}, 'bar' => {'enabled' => true}}")
  end

  it "support (deep) merge_dict function" do
    renderer = create_renderer
    renderer.compile("spec/fixtures/deep_merge_dict.j2", Hash(String, String).new).should eq("{'string' => 'b string', 'integer' => 2, 'is_true' => false, 'is_false' => true, 'old_key' => 'foo', 'hash' => {'foo' => 'bar', 'nested' => {'hash' => 'bar', 'hash1' => 'child1', 'array' => ['foo', 'bar'], 'hash2' => 'child2'}, 'bar' => 'baz'}, 'array' => ['string1', 11, true, false, 'string2', 12, false, true], 'new_key' => 'foo'}")
  end
  ...
  it "support log function" do
    ...
    renderer = create_renderer
    renderer.compile("spec/fixtures/log.j2", Hash(String, String).new).should eq("")
  ...
  it "support dictsort filter" do
    ...
    renderer = create_renderer
    output = renderer.compile("spec/fixtures/dictsort.j2", {"pillar" => data})
